...
async function loadUserProfile() {
    try {
        ...
        if (response.ok) {
            ...
            const userRole =
                user.role === "BEEKEEPER" ? "Pszczelarz" :
                    user.role === "ADMIN" ? "Administrator" :
                        "Użytkownik";

            const welcomeNameElement = document.getElementById('welcome-name');
            if (welcomeNameElement) {
                welcomeNameElement.textContent = fullName;
            }
    ...
    try {
        ...
        areas.forEach(area => {
            ...
            const availability = area.status === 'AVAILABLE'
                ? `Dostępny do ${formatDate(area.endDate || area.availableFrom)}`
...
async function fetchRentedAreas() {
    try {
        ...
        const areasWithReviewStatus = await Promise.all(areas.map(async (area) => {
            let canReview = false;
            ...
            if (area.reservationId) {
                try {
                    ...
                    console.error(`Error checking review status for reservation ${area.reservationId}:`, e);
                }
            }
            return { ...area, canReview };
        }));


        areasWithReviewStatus.forEach(area => {
            ...
            const reservationId = area.reservationId;

            // Logika wyświetlania przycisku/badge
            const reviewElement = area.reservationId
                ? (area.canReview
                        ? `<button class="btn btn-accent card-btn review-area-btn" data-reservation-id="${reservationId}">
...
async function submitEditArea(e) {
    e.preventDefault();
    ...
    let imgBase64 = document.getElementById('edit-area-current-image').src.includes('base64')
        ? document.getElementById('edit-area-current-image').src.split(',')[1]
        : null;
    ...
    const isAvailable = document.getElementById('edit-area-status-switch').checked;

    const editAreaDTO = {
        id: parseInt(areaId),
        name: document.getElementById('edit-area-title').value,
        imgBase64: imgBase64,
        type: document.getElementById('edit-area-flowers').value,
        maxHives: parseInt(document.getElementById('edit-area-max-hives').value) || 0,
        pricePerDay: parseFloat(document.getElementById('edit-area-price').value) || 0,
        description: document.getElementById('edit-area-description').value,
        endDate: document.getElementById('edit-area-date-to').value,
...
async function submitEditProduct(e) {
    ...
    const isAvailable = document.getElementById('edit-product-status-switch').checked;
...
async function submitAreaReview() {
    ...
    try {
        ...
        if (response.ok) {
            alert('✅ Dziękujemy za opinię!');
            closeAllModals();
            // WAŻNE: Odświeżamy wynajęte obszary, aby badge "Oceniono" się pojawił.
...
function setupImageUploadListeners() {
    // Obsługa zmiany zdjęcia obszaru
...
function setupEventListeners() {
    ...
    // ZAPIS ZMIAN
    document.querySelector('.submit-edit-area-btn')?.addEventListener('click', submitEditArea);
    document.querySelector('.submit-edit-product-btn')?.addEventListener('click', submitEditProduct);
